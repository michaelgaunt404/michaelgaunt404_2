---
title: Kara `r emojifont::emoji("heart")` Elon
author: 'Mike Gaunt'
date: '2023-11-14'
...
Specifically Elon and Elon affiliated stuff.  `r rep(emojifont::emoji("rocket"), 3)`
...
# library(tidytext)
# 
# person_counts_words = person_counts %>%
#   filter(flag_detect) %>% 
#   select(episode, desc) %>% 
...
#   # select(date, word) %>% 
#   pairwise_count(word, episode, sort = TRUE, upper = FALSE) %>% view()
...
# person_counts %>% 
#   mutate(desc = gsub("Send us your Listener Mail.*", "\\1", desc) %>%  
#             gsub("Visit podcastchoices.*", "\\1",.) %>% 
#            gsub("Learn more about your ad choices.*", "\\1",.) %>% 
#            str_remove_all(., "Kara and Scott talk about|Kara and Scott talk|Kara and Scott discuss")
#            ) %>% 
#   select(desc) %>%  
#   unique() %>% 
#   view()
